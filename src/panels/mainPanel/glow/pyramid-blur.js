/**
 * CPU 多尺度金字塔模糊模块
 * 使用 Float32 层、4x4 tent 降采样、双线性 Kawase 模糊
 */
...
/**
 * 构建多尺度辉光
 * 全程 Float32 精度，不在中间步骤转换为 Uint8
 * @param {{r,g,b,width,height}} sourceLayer - Float32 源遮罩层
 * @param {Object} params - 参数
 * @returns {{ glowLayer: {r,g,b,width,height} }} Float32 辉光层
 */
export function buildMultiScaleGlow(sourceLayer, params) {
  const layer = sourceLayer;

  const radiusRatio = Math.max(
    0,
    Math.min(1, Number(params.radius) / 240 || 0)
  );
  const mipCount = Math.max(
    ...
    Math.min(
      7,
      Math.floor(
        Number(params.blur.mipCount) || Math.round(3 + radiusRatio * 4)
      )
    )
  );
  const weights =
    Array.isArray(params.blur.mipWeights) && params.blur.mipWeights.length
  ...
  for (let index = levels.length - 2; index >= 0; index -= 1) {
    ...
    addLayer(
      upsampled,
      levels[index],
      weights[index] || weights[weights.length - 1] || 0.25
    );
  ...
  if (levels.length) {
    ...
    for (let index = 0; index < levels.length; index += 1) {
      addUpsampled(
        out,
        levels[index],
        weights[index] || weights[weights.length - 1] || 0.2
      );
    }
  }

  return { glowLayer: out };
}
